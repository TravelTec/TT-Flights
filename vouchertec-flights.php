...
function enqueue_scripts_flights() {   
    ...
    wp_enqueue_script( 
        'scripts-flights',
        plugin_dir_url( __FILE__ ) . 'includes/assets/js/script-search-flights.js?v='.date("dmYHis"),
        array( 'jquery' ),
        false,
        true
    );
...
function shortcode_motor_reserva_flights(){
    ...
    border-right: 1px solid #ddd;}
		...
		.qtyInc,.qtyDec, .qtyIncMulti,.qtyDecMulti{width:28px;height:28px;line-height:22px;font-size:15px;background-color:'.(empty(get_option( 'cor_flights' )) ? '#000000' : get_option( 'cor_flights' )).'bd;-webkit-text-stroke:1px #fff;color:#333;display:inline-block;text-align:center;border-radius:50%;cursor:pointer;}
		.qtyInc:hover,.qtyDec:hover,.qtyIncMulti:hover,.qtyDecMulti:hover{background:'.(empty(get_option( 'cor_flights' )) ? '#000000' : get_option( 'cor_flights' )).'bd;}
  ...
  add_menu_page( 
    ...
    function search_top_results_flights(){ 
        ...
        curl_setopt_array($curl, array(
            CURLOPT_URL => 'https://agcy-api.reservafacil.tur.br/gwaereo/v0/flights?ages='.$_POST['ages'].'&businessClass='.$_POST['businessClass'].'&packageGroup='.get_option( 'Package' ).'&routes='.$_POST['routes'].'&preferences=maxResults:120',
